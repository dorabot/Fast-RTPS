...
{
    return delegate().get_total_count();
}

int32_t InconsistentTopicStatus::total_count_change() const
{
    return delegate().get_total_count_change();
}
...
const dds::core::status::SampleRejectedState SampleRejectedStatus::last_reason() const
{
    ...
    eprosima::fastdds::dds::SampleRejectedStatusKind kind = delegate().get_last_reason();
...
{
    return delegate().get_last_instance_handle();
}
